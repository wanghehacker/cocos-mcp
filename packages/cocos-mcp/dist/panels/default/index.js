...
module.exports = Editor.Panel.define({
  ...
  `,
    ...
    methods: {
        async refresh() {
            const info = await Editor.Message.request('cocos-mcp', 'cocos-mcp:query-status');
            if (info) {
                this.$.status.textContent = info.running ? 'Running' : 'Stopped';
                this.$.status.style.color = info.running ? '#4caf50' : '#f44336';
                this.$.port.textContent = String(info.port);
                this.$.clients.textContent = String(info.clients);
            }
        },
    },
    ready() {
        ...
        self.$.btnStart.addEventListener('confirm', () => {
            Editor.Message.send('cocos-mcp', 'cocos-mcp:start');
            setTimeout(() => self.refresh(), 500);
        });
        self.$.btnStop.addEventListener('confirm', () => {
            Editor.Message.send('cocos-mcp', 'cocos-mcp:stop');
            setTimeout(() => self.refresh(), 500);
        });
